...
interface SearchPageProps {
  searchParams: Promise<{ city?: string; guests?: string; q?: string; start?: string; end?: string }>;
}
...
export default async function AkwabaPublicPage({ searchParams }: SearchPageProps) {
  ...
  // ✅ RECUPERATION DES FILTRES DE DATES
  const query = params.q || params.city || "";
  const startDate = params.start;
  const endDate = params.end;
  
  // ✅ APPEL AVEC LE NOUVEL OBJET PARAMÈTRES
  const { listings } = await searchListings({ query, startDate, endDate });
