...
import { Loader2, UploadCloud, X, CheckCircle, Copy, ExternalLink, RefreshCw, Calendar, Key } from "lucide-react";
...
export default function NewPropertyForm() {
  ...
  const [createdMode, setCreatedMode] = useState<"LONG_TERM" | "SHORT_TERM">("LONG_TERM");
  ...
  const handleCopyMessage = () => {
    const propertyUrl = createdMode === "SHORT_TERM" 
        ? `${window.location.origin}/akwaba/${createdPropertyId}`
        : `${window.location.origin}/properties/${createdPropertyId}`;
        
    const locationInfo = formData.address ? `${formData.commune} (${formData.address})` : formData.commune;
    
    const message = createdMode === "SHORT_TERM"
        ? `Bonjour, j'ai vu votre annonce pour "${formData.title}" à ${locationInfo}. Pour booster vos réservations, je l'ai mise en valeur sur Babimmo Akwaba (Court Séjour).\n\nRegardez le résultat ici : ${propertyUrl}\n\nDites-moi ce que vous en pensez !`
        : `Bonjour, j'ai vu votre annonce pour "${formData.title}" à ${locationInfo}. Pour vous aider à trouver un locataire plus vite et sécuriser vos loyers, je l'ai mise en valeur sur Babimmo.\n\nRegardez le résultat ici : ${propertyUrl}\n\nDites-moi ce que vous en pensez !`;
  ...
  if (isSuccess) {
      ...
      return (
          <div className="flex flex-col items-center justify-center py-10 text-center animate-in fade-in zoom-in duration-300">
              ...
              <p className="text-slate-500 mb-8 max-w-md">L'annonce est prête. Copiez le message ci-dessous et envoyez-le au propriétaire.</p>
              ...
              <div className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-6 mb-6 text-left relative">
                  <p className="text-sm text-slate-700 whitespace-pre-wrap font-medium">
                      Bonjour, j'ai vu votre annonce pour "{formData.title}" à {formData.address ? `${formData.commune} (${formData.address})` : formData.commune}. 
                      {createdMode === "SHORT_TERM" ? " Pour booster vos réservations, je l'ai mise en valeur sur Babimmo Akwaba." : " Pour vous aider à trouver un locataire plus vite et sécuriser vos loyers, je l'ai mise en valeur sur Babimmo."}
                      <br/><br/>
                      Regardez le résultat ici : <br/>
                      <span className="text-orange-500 font-bold break-all">{propertyUrl}</span>
                      <br/><br/>
                      Dites-moi ce que vous en pensez !
                  </p>
                  
                  <button onClick={handleCopyMessage} className="mt-6 w-full bg-slate-900 hover:bg-slate-800 text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2 transition">
                      <Copy className="w-4 h-4" /> Copier le message
                  </button>
              </div>

              <div className="flex flex-col sm:flex-row gap-4 w-full">
                  <Link href={createdMode === "SHORT_TERM" ? `/akwaba/${createdPropertyId}` : `/properties/${createdPropertyId}`} target="_blank" className="flex-1">
                  ...
                  <button onClick={handleReset} className="flex-1 bg-orange-50 text-orange-600 hover:bg-orange-100 font-bold py-3 rounded-xl flex items-center justify-center gap-2 transition">
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="flex p-1 bg-slate-100 rounded-xl mb-6">
        <button 
            type="button" 
            onClick={() => setFormData({...formData, rentalMode: "LONG_TERM"})}
            className={`flex-1 flex items-center justify-center gap-2 py-3 text-sm font-bold rounded-lg transition-all ${formData.rentalMode === "LONG_TERM" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
        >
            <Key className="w-4 h-4" /> Location Classique
        </button>
        <button 
            type="button" 
            onClick={() => setFormData({...formData, rentalMode: "SHORT_TERM"})}
            className={`flex-1 flex items-center justify-center gap-2 py-3 text-sm font-bold rounded-lg transition-all ${formData.rentalMode === "SHORT_TERM" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
        >
            <Calendar className="w-4 h-4" /> Court Séjour (Akwaba)
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-2">
          <label className="text-sm font-bold text-slate-700">
            {formData.rentalMode === "SHORT_TERM" ? "Prix par nuit (FCFA) *" : "Loyer mensuel (FCFA) *"}
          </label>
