...
export default function LeaseDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchLease = async () => {
    ...
    try {
       ...
       toast.error("Erreur chargement bail");
    } finally {
       setLoading(false);
    }
  };
  ...
  const handleTerminate = async () => {
    ...
    const result = await Swal.fire({
        ...
        text: "Le statut passera à 'TERMINATED' et le bien deviendra disponible.",
        icon: 'warning',
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            ...
            <div className="flex gap-3">
                ...
                <a 
                    ...
                    target="_blank"
                    className="flex items-center gap-2 px-4 py-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-xl transition text-sm font-bold"
                >
      ...
      >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-6">
                ...
                <Card className="bg-slate-900 border-slate-800">
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2 text-white">
                            <FileText className="text-[#F59E0B]" /> Détails du Contrat
                        </CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        <div className="flex justify-between items-center p-4 bg-slate-950 rounded-xl border border-slate-800">
                            <div>
                                <p className="text-xs text-slate-500 uppercase font-bold">Bien concerné</p>
                                <p className="font-bold text-lg text-white">{lease.property.title}</p>
                                <div className="flex items-center gap-1 text-slate-400 text-sm">
                                    <MapPin className="w-3 h-3"/> {lease.property.address}, {lease.property.commune}
                                </div>
                            </div>
                ...
                <Card className="bg-slate-900 border-slate-800">
                    <CardHeader>
                        <CardTitle className="text-white text-lg">Historique des Paiements</CardTitle>
                    </CardHeader>
                    <CardContent>
                            ...
                            {lease.payments.length > 0 ? lease.payments.map((pay: any) => (
                                <div key={pay.id} className="flex justify-between items-center p-3 hover:bg-slate-800 rounded-lg transition border border-transparent hover:border-slate-700">
                                    <div className="flex items-center gap-3">
                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center ${pay.status === 'SUCCESS' ? 'bg-emerald-500/20 text-emerald-500' : 'bg-yellow-500/20 text-yellow-500'}`}>
                                            {pay.status === 'SUCCESS' ? <CheckCircle className="w-4 h-4"/> : <Loader2 className="w-4 h-4"/>}
                                        </div>
                                        <div>
                                            <p className="font-bold text-sm text-white">Loyer {new Date(pay.date).toLocaleDateString('fr-FR', { month: 'long', year: 'numeric' })}</p>
                                ...
                                <p className="text-slate-500 text-sm italic text-center py-4">Aucun paiement enregistré.</p>
                            )}
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card className="bg-slate-900 border-slate-800 sticky top-6">
                    ...
                    <CardContent className="text-center">
                        ...
                        <div className="flex justify-center gap-2 mb-6">
                             <Badge variant="secondary" className="bg-blue-500/10 text-blue-400">VÉRIFIÉ</Badge>
                             <Badge variant="outline" className="border-slate-700 text-slate-400">BON PAYEUR</Badge>
                        </div>
