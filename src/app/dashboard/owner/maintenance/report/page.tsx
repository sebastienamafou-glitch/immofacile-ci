...
export default function ReportIncidentPage() {
  ...
  useEffect(() => {
    const fetchProperties = async () => {
        ...
        try {
            // On utilise la route properties standard
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // ✅ UTILISATION DE FORM DATA (Crucial pour l'image)
      ...
      await api.post("/owner/incidents", formData, {
          headers: { 
              'x-user-email': stored ? JSON.parse(stored).email : '',
              'Content-Type': 'multipart/form-data' // Axios le gère souvent auto, mais explicite c'est mieux
          }
      });
      
      toast.success("Ticket créé avec succès !");
      router.push("/dashboard/owner/incidents"); // ou maintenance
        ...
        <form onSubmit={handleSubmit} className="space-y-6 bg-slate-900 border border-slate-800 p-8 rounded-3xl shadow-xl">
            ...
            <div className="space-y-2">
                ...
                <div className="relative">
                    ...
                    <select 
                        required
                        className="w-full pl-12 pr-4 py-3 bg-slate-950 border border-slate-700 rounded-xl text-white outline-none focus:border-orange-500 appearance-none"
